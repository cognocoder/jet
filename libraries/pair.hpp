...
namespace jet {
  ...
  public:
    ...
    inline pair operator<<(const std::pair<std::string,std::string>& pair) const {
      this->file << pair.first << '\0' << pair.second << '\0';
      return *this;
    }

    // Retrieves a pair of strings from the file into a std::ostream.
    friend inline std::ostream& operator<<(std::ostream& os, const jet::pair& jetpair);

    // Retrieves a pair of strings from the file into a std::std::pair<std::string,std::string>.
